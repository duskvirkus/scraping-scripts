...
(async () => {
  ...
  const mainUrl = 'https://www.adidas.com/us/pride?start=48';
  ...
  urls = urls.filter(url => /^https?:\/\/www\.adidas\.com\/us\//.test(url));
  urls = urls.filter(url => /\/.{1,7}\.html/.test(url))
  urls = [...new Set(urls)]
  ...
  for (let i = 0; i < urls.length; ++i) {
    ...
    imgUrls = imgUrls.filter(url => /^https?:\/\/assets\.adidas\.com\/images\/[a-zA-Z0-9_,:]+\/[a-zA-Z0-9_]+\/.+/.test(url));
    for (let i = 0; i < imgUrls.length; ++i) {
      const regex = new RegExp(/^(https?:\/\/assets\.adidas\.com\/images\/)[a-zA-Z0-9_,:]+\/([a-zA-Z0-9_]+\/.+)/)
      const cap = imgUrls[i].match(regex);
      imgUrls[i] = cap[1] + cap[2]
    }
